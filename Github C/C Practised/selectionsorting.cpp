...
int getSmallestItemIndex(int arr[], int i , int n)
{
	int smallest=i;
	int j;
	for(j=i+1;j<=n;j++)
...
void selectinsort(int arr[], int n)
{
	...
	for(i=0;i<n-1;i++)
	{
		j=getSmallestItemIndex(arr,i,n-1);// get the index of smallest element
		temp=arr[i];
		arr[i]=arr[j];
		arr[j]=temp; //exchane the samlles element with ith element
	}
}
int main(){
